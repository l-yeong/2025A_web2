...
# 권한/보안 , 복잡한 퀴리문(집계) 결과 저장 (재사용/성능) 등등
...
# 5. 뷰 삭제
DROP VIEW IF EXISTS order_test;	# 확인방법 : 5 -> 2

# 뷰 사용시 주의할점: 특별한 경우가 아니면 select 가능하다. 뷰 에서는 update,delete,insert 조건 사용
# 뷰 는 주로 읽기 모드 사용된다.
# 수정이 불가능 뷰 : JOIN(집계) , GROUP BY , 함수 , 계산식(집계/통계) 포함 하면 읽기모드 만 가능
...
# 3. 뷰 데이터 수정
update vip_member set name = '유재석2'; # 가능
...
# 3. 뷰 데이터 수정
update view_order_summary set name = '유재석3'; # 가능
...
update vip_member2 set 집계 = 30;			# 불가능
update vip_member2 set name = '유재석4';		# 가능
